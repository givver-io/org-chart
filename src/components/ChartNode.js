...
const ChartNode = ({ datasource, NodeTemplate, draggable, collapsible, multipleSelect, changeHierarchy, onClickNode }) => {
  const node = useRef();
        ...
        {NodeTemplate ? (
          ...
          <>
            <div className="oc-heading">
              {datasource.relationship && datasource.relationship.charAt(2) === "1" && <i className="oci oci-leader oc-symbol" />}
              {datasource.name}
            </div>
